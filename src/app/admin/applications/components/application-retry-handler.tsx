...
interface ApplicationRetryHandlerProps {
...
export function ApplicationRetryHandler({
  onRetry,
  error,
  title = "Something went wrong",
  description = "An error occurred while loading application data.",
  maxRetries = 3,
  showRetryCount = true,
  variant = "card",
}: ApplicationRetryHandlerProps) {
  ...
  const handleRetry = useCallback(async () => {
    ...
    try {
      ...
      console.error("Application retry failed:", error);
    } finally {
      setIsRetrying(false);
    }
...
export function useApplicationRetryHandler(maxRetries: number = 3) {
  const [retryCount, setRetryCount] = useState(0);
  const [isRetrying, setIsRetrying] = useState(false);
