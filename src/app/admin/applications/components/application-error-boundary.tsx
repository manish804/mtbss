...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error("Application Management Error:", error, errorInfo);
    this.setState({
      error,
      errorInfo,
    });
    ...
    <div className="min-h-[400px] flex items-center justify-center p-6">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            An error occurred while loading the application management
            interface.
          </CardDescription>
        </CardHeader>
...
export function useApplicationErrorHandler() {
  const handleError = (error: Error, context?: string) => {
    console.error(
      `Application Management Error${context ? ` (${context})` : ""}:`,
      error
    );
  };
