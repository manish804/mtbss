...
import { callToActionSectionSchema } from "@/lib/validation/page-schemas";
...
type CallToActionSectionFormData = z.infer<typeof callToActionSectionSchema>;

interface CallToActionSectionEditorProps {
  data: CallToActionSectionFormData;
  onChange: (data: CallToActionSectionFormData) => void;
  onSave?: () => void;
}

export function CallToActionSectionEditor({
  data,
  onChange,
  onSave,
}: CallToActionSectionEditorProps) {
  const form = useForm<CallToActionSectionFormData>({
    resolver: zodResolver(callToActionSectionSchema),
    defaultValues: data,
    mode: "onChange",
  });
  ...
  const onSubmit = (formData: CallToActionSectionFormData) => {
    onChange(formData);
    onSave?.();
  };
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure the call-to-action section content
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Customize the appearance of the CTA section
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
